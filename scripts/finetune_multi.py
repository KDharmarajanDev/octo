...
def main(_):
    ...
    # for i, traj in enumerate(train_data_iter):
    #     # print(traj["language_instruction"])
    #     ext_1_left = Image.fromarray(np.concatenate(traj["observation"]["image_primary"].squeeze(), axis=1))
    #     ext_1_left.save(f"ext_{i}_finetune2.png")
    #     np.savetxt(f"ext_{i}_finetune2.txt", traj["action"][:, 0])
    #     np.savetxt(f"ext_{i}_finetune2_proprio.txt", traj["observation"]["state"].squeeze())
    #     # print(traj["action"])
    #     # print(traj["observation"]["proprio"])
